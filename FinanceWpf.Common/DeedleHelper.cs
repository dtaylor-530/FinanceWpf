...
namespace FinanceWpf.Common
{
    ...
    public static class DeedleHelper
    {

            public static IEnumerable<KeyValuePair<DateTime, double>> ToKVPairs(this Series<DateTime, double> series)
            {
                foreach (var s in series.Observations)
                {
                    yield return new KeyValuePair<DateTime, double>(s.Key, s.Value);
                }

            }

        public static Series<DateTime, double> GetData(string fileName, string column, string datecolumn)
        {
            var frame = Deedle.Frame.ReadCsv(fileName);
            var frameDate = frame.IndexRows<DateTime>(frame.ColumnKeys.SingleOrDefault(_ => string.Equals(_, datecolumn, StringComparison.CurrentCultureIgnoreCase)))
                .SortRowsByKey();
        ...
        //public static Series<DateTime, double> GetData<T>(IEnumerable<T> data, string column, string datecolumn)
        //{
        //    var frame =new Deedle.SeriesBuilder<double>().
        //    var frameDate = frame.IndexRows<DateTime>(frame.ColumnKeys.SingleOrDefault(_ => string.Equals(_, datecolumn, StringComparison.CurrentCultureIgnoreCase)))
        //        .SortRowsByKey();
    ...
    static class DeedleTimeSeriesSamples
    {
        public static void Samples([CallerFilePath] string file = "")
        {
            var root = System.IO.Path.GetDirectoryName(file);
        ...
        { 1, "one" }, { 2, "two" }, { 3, "three" } }.Series;
            ...
            // [create-csv]
            var frame = Frame.ReadCsv(System.IO.Path.Combine(root, "../data/stocks/msft.csv"));
